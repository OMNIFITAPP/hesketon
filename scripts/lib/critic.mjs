...
1. חזרה סמנטית בין שכבות הפתיחה. אם אותו רעיון מופיע בציטוט הפותח, באמ;לק, בפתיחה ובפרק הראשון — הוא נשאר במקום אחד, והשאר משוכתבים כך שכל שכבה תעשה תפקיד אחר (הציטוט מסקרן; האמ;לק ממפה; הפתיחה מציבה מתח; הפרק הראשון מתחיל להסביר). המבחן: הקורא מתקדם, לא מקבל את אותו מסר בניסוחים משתנים.

2. חזרה רעיונית בגוף. לכל פסקה שאל: מה המידע החדש כאן? אין — מזג או מחק. חזרה היא חזרה גם כשהמילים שונות לגמרי.

3. עומק מדומה. משפטים שנשמעים משמעותיים אך ריקים ("זהו רצף הרגשות שהופך קבוצה לקבוצה", "מתנה שכל אדם מחזיק בה", "עיקרון אוניברסלי") — מחק או תרגם לפעולה קונקרטית. סיום פילוסופי-השראתי מוחלף בחזרה לתזה בתמונה מוחשית.

4. הגבהת מעמד. התנסות אישית שכונתה "ניסוי", תגובה של אדם אחד שהוצגה כ"ממצא", "הוכיח" בלי מחקר, מתאם שהפך סיבתיות — הורד לקרקע, כולל בכותרות הביניים ("הניסוי שערך X" → "הרגע שבו X ניסה את זה בעצמו").

5. כותרות ביניים שקובעות יותר מהגוף. כותרת שמציגה את עמדת המרואיין כקביעה של הבלוג, או מבטיחה יותר ממה שהפרק מקיים — רכך לדיוק.

6. ניפוח לאורך. פסקה שקיימת רק כדי להאריך (חזרה על התזה, מתיחת מטאפורה, הסבר של מה שכבר הובן, פרק צדדי שלא משרת את התזה) — מחק. טקסט קצר וצפוף גובר על ארוך ומדולל.

7. מקצב מכני. פסקאות באורך אחיד, שורת מחץ בסוף כל פרק, רצף קבוע של טענה→הסבר→פאנץ' — שבור את הסימטריה: גוון אורכים, מזג פסקאות קצרות, מחק שורות מחץ מיותרות. סיום פרק לא אמור להישמע כמו ציטוט לאינסטגרם.

8. תחביר שבור או מתורגם שמסתיר תמונה לא ברורה — נסח מחדש לתמונה אחת חדה.
